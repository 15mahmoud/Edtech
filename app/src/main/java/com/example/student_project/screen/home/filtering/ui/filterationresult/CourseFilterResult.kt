...
fun CourseFilterResultScreen(
    ...
    val newCourseList = state.value.course.filter { course ->
        course.category == courseCategory && course.difficulty == difficultyLevel && course.releasedDate == releasedDate && course.rating.toFloat() >= rating!! && course.hourlyRate.toFloat() <= hourlyRate!!
    }
    ...
    Text(text = "All Courses", style = MaterialTheme.typography.headlineLarge, fontSize = 15.sp, modifier = Modifier.padding(start = 15.dp, bottom = 5.dp,top=5.dp))
            LazyColumn(modifier = Modifier.padding(start = 15.dp)) {
                items(newCourseList) {
                    CourseColumn(course = it, onClickListener = { string ->
        ...
        Row(modifier = Modifier.fillMaxWidth()) {
            ...
            Column {
                ...
                Text(
                    ...
                    fontSize = 15.sp,
                    modifier = Modifier.padding(5.dp), color = jopTitleColor
                )
                HorizontalDivider()
                Row {
                    ...
                    Text(
                        ...
                        style = MaterialTheme.typography.titleMedium,
                        fontSize = 15.sp,
                        modifier = Modifier.padding(5.dp), color = buttonColor
                    )
